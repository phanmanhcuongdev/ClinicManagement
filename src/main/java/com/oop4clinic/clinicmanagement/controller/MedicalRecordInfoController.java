...
public class MedicalRecordInfoController {
    ...
    private void loadMedicalRecords() {
        try {
            List<MedicalRecordDTO> records = medicalRecordService.getMedicalRecordsForDoctor(loggedInDoctor.getId());
            medicalRecordList.setAll(records);
            searchField.clear();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void handleShowPatientInfo(ActionEvent event) {
        try {
            List<AppointmentDTO> allAppointmentsToday = appointmentService.getAppointmentsForToday(loggedInDoctor.getId());
