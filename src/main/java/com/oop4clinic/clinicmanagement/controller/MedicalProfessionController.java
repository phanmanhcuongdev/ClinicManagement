...
public class MedicalProfessionController {
    ...
    private void loadAppointmentsForDate(LocalDate date) {
        if (loggedInDoctor == null) {
            System.err.println("⚠ Chưa có thông tin bác sĩ đăng nhập.");
            return;
        }
        int loggedInDoctorId = loggedInDoctor.getId();
        try {
    ...
    private void handleShowAll(ActionEvent event) {
        ...
        try {
            ...
            List<AppointmentDTO> allAppointments = appointmentService.getAllAppointmentsForDoctor(loggedInDoctor.getId());
    ...
    private void handleShowPatientInfo(ActionEvent event) {
        ...
        try {
            ...
            List<AppointmentDTO> allDoctorAppointments = appointmentService.getAllAppointmentsForDoctor(loggedInDoctor.getId());
    ...
    private void handleShowMedicalRecordInfo(ActionEvent event) {
        ...
        try {
            ...
            recordController.setLoggedInDoctor(loggedInDoctor);
    ...
    private void handleShowMyProfile(ActionEvent event) {
        if (loggedInDoctor == null) {
            showAlert(Alert.AlertType.ERROR, "Lỗi", "Không có thông tin bác sĩ đăng nhập.");
            return;
        }
        try {
            ...
            profileController.setLoggedInDoctor(loggedInDoctor);
    ...
    private void openMedicalRecordDialog(AppointmentDTO appointment, MedicalRecordDTO record) {
        ...
        try {
            ...
            controller.setLoggedInDoctor(this.loggedInDoctor);
